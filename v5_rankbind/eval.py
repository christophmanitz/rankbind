...
from v5_rankbind.data import (
    ensure_chemberta_cache, prepare_frames, build_datasets,
    collate_pointwise, load_chemberta,
)
...
def main() -> None:
    ...
    df = run_test_set(model, test_ds, device)
    ...
    M, lig_list, prot_list = build_score_matrix(
        model, cfg, device, n_matrix=cfg["eval"]["n_matrix"],
    )
