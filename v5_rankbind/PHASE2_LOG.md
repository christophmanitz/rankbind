...
Phase 2 is fully shipped. Phase-4 Stages (c) and (b) ran 2026-04-27 and are
complete. Stage (a) was empirically blocked by the (b) attention-weight
inspection and is deferred indefinitely. Authoritative plan addendum:
`PLAN.md §14`.
...
- **Stage (c): v4 failure-case diagnosis** (PLAN.md §13.1).
  ...
  Verdict: **mixed, proceed to (b), defer (a).**
...
- **Stage (b): Residue attention-pool** (PLAN.md §13.2).
  ...
  **3-seed numbers (seeds 42, 7, 1337):**
  ...
  Param delta vs v4: **+3,840 (+0.6%)**, a representational change, not a
  capacity change. MRR-arm of §13.2 gate passes by 2× the threshold.

  **But:** the seed range tells a stability story. attn_pool seeds 42/7/1337
  produce MRR 0.316 / 0.559 / 0.405; std grew 1.7× (0.072 to 0.123).
  The headline lift is real, but s7 is an outlier-high and s42 sits
  essentially on the v4 mean.
...
- **Matched-capacity ablations (v4, 3-seed):** margin loss is the
  dominant contribution (removing it: MRR 0.326 to 0.041); balanced
  sampler is a secondary positive (MRR 0.326 to 0.183); bilinear vs MLP
...
- `v5_rankbind/configs/default.json`
  - `triplet.negative_sampling`: `"cross_protein_implicit"` to `"hard"`.
...
- `matrix_mrr ≥ 0.10`: pass (0.247)
- `matrix_hit_at_10 ≥ 0.15`: pass (0.647)
- `Gini-residual ≤ −0.01`: pass (−0.208)
- `Top-10 Jaccard vs null_prot_prior ≤ 0.30`: pass (0.000)
- `Global AUC ≥ 0.80`: fail (0.623). Gate retired per Priority C
...
uses random, the cache is not yet built):
...
- **default (bal+margin+biln)**: hard negs lift MRR +23%, H@5 +21%,
  H@10 +16%. Gini-residual improves −0.124 to −0.208. Clear win. This
  is the reported v4 recipe.
- **abl_no_sampler**: MRR about flat (−5%), but H@10 jumps +63% (0.324
  to 0.529) and Gini-residual improves −0.072 to −0.100. Interpretation:
  hard-neg mining sharpens the near-ranking region; the balanced
  sampler is still needed for exact top-1.
- **abl_no_bilinear**: MRR drops 0.220 to 0.173 (−21%), H@5 −11%, H@10
  −16%. But gAUC rises 0.666 to 0.678, Gini-residual deepens −0.251 to
  −0.276, and best val epoch moves 32 to 26 (faster convergence).
...
- `v5_rankbind/train.py`: added `--seed` CLI flag. When set, overrides
  ...
  note `seed override: cfg=<X> to cli=<Y>` so the actual seed is
...
- `scripts/run_v5_multiseed.sh` (new): thin loop `configs × seeds`
  calling `run_v5_rankbind.sh`. Default matrix: 5 configs × {7, 1337} =
  10 jobs. Accepts an optional single-config arg. Not used for the first
  sweep (because `default@7` was already smoke-tested separately, see
  below), but kept for future re-runs.
...
reseeded), and the `seed override: cfg=42 to cli=7` note is present.
...
**Context.** I pushed back on the "Global AUC ≥ 0.80 retired" framing.
...
- `model.py`                `LigandProjector` + `ProteinProjector` (frozen
                            embeddings to 256-d), `BilinearHead` (low-rank
...
**0.4%** of positives beat max(neg). Train loss collapses 0.24 to 0.02 over
...
**Thresholds from PLAN.md §evaluation:**
- `matrix_mrr ≥ 0.10`: pass, 3 of 3 margin configs
- `matrix_hit_at_10 ≥ 0.15`: pass, 3 of 3 margin configs
- `Gini-residual ≤ −0.01`: pass, all five pass (even abl_bce_only at −0.002)
- `Top-10 Jaccard vs null_prot_prior ≤ 0.30`: pass, 4 of 5 (only Phase-1
  equivalent abl_bce_only fails at 0.429, as expected)
- `Global AUC ≥ 0.80`: fail, margin configs 0.57-0.67. **This threshold was
...
- **Margin loss is the dominant Phase-2 contribution.** Remove it and MRR
  drops 9-17× (0.20 to 0.02), H@10 drops ~15× (0.56 to 0.03).
...
- **BCE configs reproduce Phase-1 pathology regardless of architecture.**
  ...
  matrix MRR 0.013, i.e. it learns the protein-level shortcut perfectly.
...
### B. Multi-seed runs
Three seeds × (default, abl_no_sampler, abl_no_margin, abl_no_bilinear) =
12 jobs × ~10 min each ≈ 2 GPU-hours. Adds error bars to the ablation
table. Submit after A lands so we only re-run once. Seeds: 42 (current),
7, 1337.
...
- Explicitly promote matrix MRR and H@10 to primary, per_ligand_auc to supp.
...
file references `results/v5_rankbind/current/score_matrix_rankbind.npy`;
we can add a `current` symlink pointing to the chosen v3 run, or hardcode
the path for paper figures.
