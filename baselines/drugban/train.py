"""
baselines/drugban/train.py — Train DrugBAN on BRENDA hydrolase data.

DrugBAN (Bai et al., 2023): GCN on ligand graph + CNN on protein sequence + BAN fusion.
Adapted for binary binding classification.

Usage (see run_drugban.sh):
  python baselines/drugban/train.py
...
def train():
    ...
    mat_path = os.path.join(OUT_DIR, 'score_matrix_DrugBAN.npy')
    np.save(mat_path, score_matrix)
    np.save(os.path.join(OUT_DIR, 'true_prot_idx_DrugBAN.npy'), np.arange(n_act))
