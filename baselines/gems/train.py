"""
baselines/gems/train.py — Train GEMS on BRENDA hydrolase data.

GEMS (camlab-ethz, 2024): BiLSTM on protein sequence + GCN on ligand graph.
Adapted for binary binding classification.

Usage:
  python baselines/gems/train.py
...
def train():
    ...
    mat_path = os.path.join(OUT_DIR, 'score_matrix_GEMS.npy')
    np.save(mat_path, score_matrix)
    np.save(os.path.join(OUT_DIR, 'true_prot_idx_GEMS.npy'), np.arange(n_act))
