"""
baselines/graphdta/train.py — Train GraphDTA on BRENDA hydrolase data.

GraphDTA (Nguyen et al., 2020): GCN on ligand molecular graph + CNN on protein sequence.
Adapted for binary binding classification.

Usage (see run_graphdta.sh):
  python baselines/graphdta/train.py
"""
...
from model import GraphDTA, ProteinTokenizer
from baseline_dataset import SeqBaselineDataset, collate_seq, LigandTokenizer
...
def train():
    ...
    for epoch in range(1, EPOCHS + 1):
        model.train()
        total_loss = 0.0
        for prot_ids, lig_ids, mol_batch, ys in train_loader:
            prot_ids  = prot_ids.to(device)
            mol_batch = mol_batch.to(device)
            ys        = ys.to(device)

            prob = model(prot_ids, mol_batch)
        ...
        val_loss = 0.0
        with torch.no_grad():
            for prot_ids, lig_ids, mol_batch, ys in val_loader:
                prot_ids  = prot_ids.to(device)
                mol_batch = mol_batch.to(device)
                prob = model(prot_ids, mol_batch).cpu()
                val_loss += criterion(prob, ys).item() * len(ys)
                all_probs.extend(prob.numpy())
                all_labels.extend(ys.numpy())
        val_loss /= n_val

        auc = roc_auc_score(all_labels, np.array(all_probs)) if np.sum(all_labels) > 0 else 0.0
        ap  = average_precision_score(all_labels, np.array(all_probs)) if np.sum(all_labels) > 0 else 0.0
    ...
    prot_ids_all = torch.stack([it[0] for it in items])       # [N, L_prot]
    mol_graphs   = [it[2] for it in items]

    score_matrix = np.zeros((n_act, n_act), dtype=np.float32)
    with torch.no_grad():
        for i in range(n_act):
            p = prot_ids_all[i:i+1].expand(n_act, -1).to(device)
            m = Batch.from_data_list(mol_graphs).to(device)
            score_matrix[i] = model(p, m).cpu().numpy()

    mat_path = os.path.join(OUT_DIR, 'score_matrix_GraphDTA.npy')
    np.save(mat_path, score_matrix)
    np.save(os.path.join(OUT_DIR, 'true_prot_idx_GraphDTA.npy'), np.arange(n_act))
