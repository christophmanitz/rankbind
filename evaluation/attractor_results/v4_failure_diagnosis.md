...
|---|---:|---:|---|
| n_positive_pairs_matched | 34 | 34 | yes |
| MRR | 0.2465 | 0.2465 | yes |
| H@1 / H@5 / H@10 | 0.029 / 0.500 / 0.647 | 0.029 / 0.500 / 0.647 | yes |
...
Spearman ρ(`n_heavy_atoms`, `rank`) = 0.252 (weak positive, threshold 0.4
not reached).
...
| `CC(=O)Oc1ccc([N+](=O)[O-])cc1` | 33 | p-Nitrophenyl acetate, an aryl ester. |
| `CCCC(=O)Oc1ccc([N+](=O)[O-])cc1` | 11 | p-Nitrophenyl butyrate, an aryl ester. |
| `N=C(N)NCCCCN` | 2 | Agmatine, guanidine + amine; no SMARTS for guanidine. |

If we re-tag, 4 of the 6 are aryl esters or amide-tautomers, both of
...
"large-ligand means bad rank" story has a long-tail flavour that is real
...
> `n_heavy_atoms` to `rank` correlation.
> Fail: failures chemically flat, no size correlation.
> Mixed: no class cluster but residue-level interpretability arguments
> still hold.
...
This is a textbook Mixed outcome.
...
3. Stage (b), the residue attention-pool, is justified independently of
...
- `evaluation/v4_failure_diagnosis.py`, the script
