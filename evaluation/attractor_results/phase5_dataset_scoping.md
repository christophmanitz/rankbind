...
**Date:** 2026-04-27
**Goal:** Identify a realistic external enzyme-substrate dataset on which
to evaluate `RankBind` (trained on BRENDA) under distribution shift —
the test of the Phase-2 + Stage-(b) thesis, not just another ablation.
...
1. **Pair structure:** (protein, small-molecule) tuples with a
   binary or thresholdable binding label. Continuous kcat is acceptable
   if we threshold (e.g. log10 ≥ −2 → "binder").
2. **UniProt IDs:** so we can reuse the existing
   `data/esm2_embeddings/` cache without recomputing PLM embeddings on
   thousands of new sequences.
...
5. **License:** redistributable / non-commercial use is fine for an
   academic publication; closed-source datasets are out.
6. **Size:** ≥ 500 protein-substrate pairs in the post-overlap-filter
   set, ≥ 100 unique proteins. Smaller and the n-too-thin problem from
...
- **Pros:** Largest, binary-label, cleanly aligned with our task
  ...
  test splits.
- **Cons:** **High BRENDA overlap risk** — KEGG and BRENDA are both
  source corpora. Without a per-pair UniProt-overlap filter, this is
  not a true cross-dataset probe. Required: filter out any pair where
...
- **Net:** Second-choice. Use as a **complementary** probe alongside
  ESP if both yield clean post-filter sets.

### 2.3 DLKcat (Li et al. 2022)

- **Size:** ~17,000 (enzyme, substrate, kcat) tuples.
- **License:** open.
- **Cons:** **Heavy BRENDA overlap** — DLKcat is largely a BRENDA
  re-export plus literature additions. Not a clean external probe by
...
- **Cons:** Kinases are over-represented in BRENDA training data
  ...
  expected. Also: kinase-inhibitor ≠ enzyme-substrate semantically —
  inhibitors compete with substrates, not bind as substrates.
...
|---|---:|:---:|:---:|---|:---:|---|---|
| **ESP** | ~280k | ✅ | ✅ | open | ✅ | high — filter required | ✅ Primary |
| **TurNuP** | ~15k | ✅ | ✅ | open | ✅ (kcat-thresh) | medium | ✅ Secondary |
| DLKcat | ~17k | ✅ | ✅ | open | ✅ | very high (BRENDA-derived) | ❌ Reject |
| BindingDB | ~2.8M | ✅ | ✅ | open | partial | unknown | ⚠️ Appendix only |
| DAVIS / KIBA | ~30k | ✅ | ✅ | open | partial (kinases) | high | ❌ Reject |
| ChEMBL | ~20M | ✅ | ✅ | open | partial | unknown | ⚠️ Appendix only |
...
## 5. Decision-gate (probe → paper-claim)
...
- ⚠️ **Partial pass:** lift is in the right direction but small
