...
top proteins. Pooled AUC in this regime certifies protein popularity, not
binding. We introduce \method, a 627k-parameter model that optimises
...
from 0.015 (binary cross-entropy control) to $0.326 \pm 0.072$ (three seeds)
and cuts the top-ten overlap with the molecule-blind baseline from
$0.54$--$0.67$ to $0.035$. The pattern is not a BRENDA artefact. A matched
...
than unbound ones. That question can be answered without looking at the
molecule. If some proteins have many positive pairs in the training data and
others have none, a model that scores every pair of a popular protein high
will pass pooled AUC, no matter how it treats the molecule. That is an
...
top-ten most-scored proteins. In this regime, pooled AUC certifies the
protein marginal, nothing else. Gini, which we originally suspected of
detecting a learned pathology, turns out to be a dataset-geometry artefact:
the molecule-blind baseline produces it without any training.
...
seeds on the same split lift \mrr{} from $0.015$ for a binary cross-entropy
(BCE) control to $0.326 \pm 0.072$, and the per-molecule ranking AUC from
chance to $0.891$. The ablation isolates the margin loss as the dominant
lever: removing it collapses \mrr{} by a factor of eight and pooled AUC
rebounds to $0.95$ (the shortcut returns the moment the ranking objective
is gone).

Section~2.3 adds a residue-level attention pool over per-residue ESM2
embeddings ($+0.6\%$ parameters, $+0.10$ absolute \mrr{}), together with an
...
\subsection{Pooled AUC certifies protein popularity on BRENDA-200}
\label{sec:shortcut}
...
version rests on $n=4$. \method\ values are three-seed means; its
per-molecule AUC is seed-42.}
\label{tab:phase1}
\small
...
\midrule
\method\ (v4, 3 seeds) & 0.634 & 0.891 & 0.035 & 0.787 \\
\bottomrule
\end{tabular}
\end{table}
...
marginal, and pooled AUC rewards them for it. The response maps in
...
\method's matrix is de-striped (Gini $0.787$).
...
\begin{table}[t]
\centering
\caption{Three-seed means $\pm$ s.d.\ on BRENDA-200, ordered by \mrr{}.
The top row is a matched-capacity BCE control with the same encoders and
split. Removing the margin loss collapses \mrr{} by $8\times$ and pooled
AUC rebounds to $0.95$: the shortcut returns. Removing the balanced sampler
costs $44\%$. The MLP head matches the bilinear head's mean but has
$2.2\times$ wider seed spread.}
\label{tab:ablation}
\small
...
Config & \mrr{} & Hit@10 & Pooled AUC & Gini-res. & Top-10 \\
       &        &        &            &           & overlap \\
\midrule
BCE control (MLP head)        & 0.015$\pm$0.002 & 0.000 & 0.948$\pm$0.030 & $-0.002$ & 0.587$\pm$0.137 \\
$-$ margin loss               & 0.041$\pm$0.023 & 0.098 & 0.948$\pm$0.028 & $-0.043$ & 0.018 \\
$-$ balanced sampler          & 0.183$\pm$0.060 & 0.422 & 0.630$\pm$0.038 & $-0.074$ & 0.037 \\
$-$ bilinear head (MLP)       & 0.243$\pm$0.161 & 0.520 & 0.660$\pm$0.040 & $-0.182$ & 0.018 \\
\textbf{\method\ v4}          & \textbf{0.326$\pm$0.072} & 0.755$\pm$0.095 & 0.634$\pm$0.010 & $-0.210$ & 0.035$\pm$0.030 \\
$+$ residue attention pool    & \textbf{0.427$\pm$0.123} & 0.814$\pm$0.103 & 0.659$\pm$0.028 & $-0.216$ & 0.018 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ablation} orders the ingredients by what they buy. The margin
loss is the dominant contribution: without it \mrr{} falls from $0.326$ to
$0.041$ and pooled AUC jumps back to $0.95$ (the model stops ranking and
re-learns the protein marginal). The balanced sampler is a secondary positive
($-44\%$ when removed). The bilinear head does not win on the mean
($0.243$ for MLP versus $0.326$ for bilinear is partly seed luck) but cuts
the seed-to-seed standard deviation from $0.161$ to $0.072$; we keep it for
stability, not for a mean win. Hard-negative mining, which is part of the
default rather than an ablation, lifts \mrr{} from $0.201$ (v3, random
negatives) to $0.326$ ($+62\%$).
...
baselines' pathology in full: pooled AUC $0.948$, \mrr{} $0.015$, top-ten
overlap with the molecule-blind prior $0.587$. The shortcut behaviour is
...
over the per-residue embeddings, adding 3{,}840 parameters ($+0.6\%$). The
three-seed result is \mrr{} $0.427 \pm 0.123$ (from $0.326 \pm 0.072$),
Hit@10 $0.814$, per-molecule AUC $0.930$ (seed 42).
...
one of them built on pretrained protein language models, all pass it by
reproducing the per-protein label marginal, and a molecule-blind baseline
passes it just as easily. The fix is not a better encoder or a bigger
...
control, \method{} trades pooled AUC down from $0.95$ to $0.63$ while \mrr{}
goes from $0.015$ to $0.326$. We consider this the right trade: the high
...
Three limitations bound the claims. First, the transfer numbers are
single-seed; the BRENDA-200 headline is three-seed, the enzyme-wide peaks
are not yet. Second, the BRENDA decoy protocol assigns each molecule a
...
$0.634$, and the seven-dataset control comparison is the cleaner
...
pooled AUC with patience 10 (minimum 20 epochs). The balanced sampler draws
...
(Sections~2.1--2.3); the transfer runs of Section~2.4 are single-seed (seed
42). Hardware: one NVIDIA A30 GPU per run, about 1.5\,h per seed for the
