...
are not yet. Second, the decoy construction of the BRENDA corpus leaks
pair-level statistics into the inputs (a linear probe on the frozen
embeddings reaches pair-level AUC $\approx 0.86$ before any training;
\method{} removes most of this signal, leaving $0.68$), so absolute pooled
values on this corpus should be read with that in mind; the seven-dataset
control comparison is the cleaner instrument. Third, the residue-level
