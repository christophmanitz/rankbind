...
- **Phase 1**: done. Four baselines trained + evaluated; null-baseline pivot
  refuted the Gini-attractor thesis. Narrative/figures:
  `evaluation/attractor_results/phase1_report.html`.
- **Phase 2**: `v5_rankbind/` package built, matched-capacity ablation done,
  **hard-negative mining (Priority A) + ablation re-runs (A1) + multi-seed
  sweep (B) all landed 2026-04-23 as tag v4**. Remaining work is
  documentation-only (Priority C — PLAN.md addendum; D — figure
  regeneration; E — Phase-2 HTML report). Authoritative delta:
  `v5_rankbind/PHASE2_LOG.md` — the top "Resume state" section dated
  2026-04-23 is the current state; the older 2026-04-22 section is
  historical.
- **gAUC retirement decision confirmed by probe** (2026-04-23 ~16:00):
  A BCE-auxiliary probe (`bce_aux_weight=0.5`, seed=42) lifted test
  gAUC by only +0.03 (0.623 → 0.655) — nowhere near 0.80. Matrix
  ranking unchanged. Confirms the 0.80 threshold is a dataset ceiling
  under ligand-conditional optimisation, not a loss-function artefact.
  `Global AUC ≥ 0.80` stays retired **as a success gate**; gAUC is
  still **reported** in every table. Evidence: run_dir
...
- `CLAUDE.md` (this file) + `v5_rankbind/PHASE2_LOG.md` — the authoritative
  deltas. When they conflict, PHASE2_LOG wins for Phase-2-specific
  guidance.

Phase 1 was executed but the original framing (Gini-based universal attractor
bias) did not survive a null-baseline probe. The empirical findings
**reframe** Phases 2–5:
...
- `v5_rankbind/PHASE2_LOG.md` — Phase-2 session log: what's built,
...
- Leipzig HPC cluster. Primary venv: `~/venvs/hieratombind`
  ...
  - **Rebuilt 2026-04-29** after the original venv was deleted to free disk.
    ...
    `rdkit`. **`torch_geometric` is NOT installed** — only the v5_rankbind /
...
therefore a **data-geometry artifact**, not evidence of learned pathology.
The real pathology is visible on a different axis: all four models achieve
high global AUC (0.63 – 0.95) but **per-ligand AUC ≤ 0.625** — three of the
four score below random (0.5). Models learn protein-level shortcuts, not
...
4. **Hard-negative mining (v4)** — `v5_rankbind/sampler.py::TripletCollator`:
   ...
   learns to separate its own hardest confusers (0.92 → 0.97 over 32 epochs
   in v4 default).
...
- **Bilinear head is strictly more stable than MLP under hard negs.**
  Multi-seed: bilinear MRR std 0.072, MLP std 0.161 — the MLP head
...
- **C. `v5_rankbind/PLAN.md` addendum** ← start here. No cluster time
  ...
  to primary, cite the multiseed CSV as canonical numbers.
...
- `v5_rankbind/sampler.py` — `ProteinBalancedSampler`, `TripletCollator`
  (currently `cross_protein_implicit` negatives — hard-negative variant is
  the Priority-A work)
...
- Tag every SLURM run: `bash scripts/run_v5_rankbind.sh <cfg> paula <tag>`.
  Current tag progression: v1 (broken collator) → v2 (first valid) → v3
  (matched capacity) → **v4 (hard-negative mining — reported numbers,
  seed=42 canonical)** → **v4_s7 / v4_s1337 (multi-seed error bars)**.
  For a seed sweep: 4th arg is the seed, e.g.
