# ResidueOnlyBind → RankBind: Development Plan

**Goal:** Evolve the current ResidueOnlyBind model into a publishable architecture that addresses the protein-attractor bias problem with a ranking-normalised, granularity-adaptive framework.

**Paper thesis:** Naive dual-encoder models for protein–ligand binding prediction suffer from protein-attractor bias — a failure mode where structurally promiscuous proteins dominate top-k predictions regardless of the query ligand. We propose RankBind, a ranking-normalised model that combines pretrained embeddings with structure-aware GNNs and adaptive atom-level re-integration, and demonstrate that this failure mode is widespread across existing architectures.

**Core design principle:** Pretrained encoders (ESM2, Uni-Mol) provide rich **node and edge features** for the GNN inputs. The GNNs remain the core architecture — they propagate these features over 3D structural graphs (Cα contacts for proteins, atom bonds for ligands). This is the key distinction from pure fine-tuning approaches: ESM2 alone has no 3D structure awareness, and Uni-Mol alone has no protein–ligand interaction modelling. The GNNs add structural reasoning on top of pretrained chemical/biological knowledge.
...
**Why first:** Establishing that the attractor problem is not specific to our model but a widespread failure mode across architectures gives the paper its central scientific contribution. This turns a limitation into a research question. It also provides the reference metrics (ROC-AUC, Hit@k) on our BRENDA dataset before any architectural changes, and informs the design of the ranking loss in Phase 3.
...
Design a standardised evaluation protocol that exposes attractor bias:

1. **Response map construction:** For each model, score every (ligand, protein) pair in the validation set → heatmap matrix
...
3. **Gini coefficient of attractor scores:** Measures concentration — Gini = 0 means uniform, Gini → 1 means a few proteins dominate
4. **Rank displacement:** For each ligand, measure `rank(true_protein) - rank(true_protein | excluding attractors)` — quantifies how much the attractor shifts the true partner's rank
5. **Score variance per ligand:** Low variance = model predicts similar scores for all proteins (no discrimination); high variance = model is selective
...
- **Prediction by architecture type:**
  ...
  - Dual-loss (DualBind NVIDIA): unclear — DSM loss may implicitly regularise against attractors
- The response map heatmaps should show vertical bright bands for all models, not just ours
- Models with explicit cross-attention (DrugBAN, GIGN) may show less attractor bias — this supports our argument for ranking-based correction
...
- **Narrative:** This section forms the "problem statement" of the paper — it justifies why RankBind is needed

### 1.5 Dataset Suitability Analysis

Before running all baselines, verify that the BRENDA hydrolase dataset is well-suited for exposing attractor bias. A dataset where attractor bias cannot manifest (e.g., all proteins are equally represented and structurally distinct) would not be informative. The following checks should be performed:

- **Protein representation imbalance:** Compute the distribution of ligands per protein. The current dataset has extreme skew (Protein 22: 38 ligands vs. several with 1–2). This imbalance is a prerequisite for attractors — proteins with many training examples have more opportunity to develop inflated baselines. Quantify with Gini coefficient over ligand counts.
- **Structural similarity between proteins:** Compute pairwise sequence identity (e.g., via MMseqs2) and structural similarity (TM-score via TM-align or Foldseek) across all ~400 proteins. High similarity clusters indicate regions where attractor bias is most likely — models will struggle to distinguish proteins within the same structural cluster. Visualise as a dendrogram or similarity heatmap.
- **EC sub-class overlap:** Check how many proteins share the same EC sub-class (e.g., multiple EC 3.1.1.* esterases). Proteins within the same EC group are functionally similar and more likely to become attractors for each other's ligands.
- **Ligand chemical diversity:** Compute Tanimoto similarity matrix across all ligands. If many ligands are chemically similar, the model has less discriminative signal to assign them to different proteins, amplifying attractor effects.
- **Minimum viable dataset properties:** Define thresholds — e.g., the dataset should have ≥ 20 proteins with ≥ 3 ligands each for meaningful attractor analysis. Check if the validation split alone meets these criteria, or if cross-validation is needed.
- **Comparison to standard benchmarks:** If possible, run the same suitability checks on PDBbind or BindingDB subsets to contextualise whether our dataset is more or less prone to attractor bias than typical benchmarks.

If the dataset turns out to have insufficient protein diversity for a convincing attractor analysis, consider expanding to additional EC classes (beyond EC 3.*) or supplementing with BindingDB entries before training the baselines.

### Deliverables
- [ ] Training scripts for 5–7 baselines on BRENDA hydrolase data (same splits, same decoys)
- [ ] `attractor_diagnosis.py` — standardised evaluation script producing all metrics + heatmaps
...
### 2.1 Protein Side: ESM2 → ProteinGraphTransformer Node Features

ESM2 is a protein language model pretrained on ~250M sequences. It produces 1280-dim per-residue embeddings that capture evolutionary and biochemical context — but it has **no 3D structural awareness**. The ProteinGraphTransformer adds exactly this: it propagates ESM2 features over the Cα contact graph (8 Å cutoff), enabling the model to learn structure-dependent residue interactions that ESM2 alone cannot capture.
...
- Script: `precompute_esm2.py` — batch processing on GPU, ~2–3 hours for ~400 proteins on A30
- Fallback: if sequence > 1024 residues, use sliding window with overlap and average

#### 2.1.2 Modify ProteinGraphTransformer Input
- **Node features:** Replace 33-dim handcrafted → 1280-dim ESM2 embeddings
...
- **Ablation variant:** Concatenate ESM2 (1280) + handcrafted (33) → 1313-dim, letting the model learn which signals are complementary
...
- **ESM2-only baseline:** Pool ESM2 residue embeddings (mean/attention) → protein vector, no GNN. This captures sequence-level information but misses 3D contacts.
...
- **Expected:** ESM2+GNN > ESM2-only, because proteins with similar sequences but different folds (or vice versa) will be better discriminated. This justifies keeping the GNN as core architecture.

### 2.2 Ligand Side: Enriched Atom Features → LigandGraphTransformer
...
- The ligand GNN converges well and gradient norms are balanced — it is not the bottleneck
...
- If attractor bias is solved by Phase 3 ranking loss, ligand features may not need improvement
...
- Replace 25-dim node features with 512-dim Uni-Mol embeddings in the LigandGraphTransformer
...
- **Important:** Uni-Mol provides atom-level features, but the GNN adds bond-topology reasoning and interaction with the protein encoder via the fusion module

**Option C — Hybrid enrichment:**
- Concatenate original 25-dim + Morgan fingerprint bits (2048→64 via learned projection) → 89-dim
...
Start with Option A (unchanged). After Phase 3, if ligand-side encoding is still a bottleneck (visible in case studies), add Option B as an ablation. This keeps the experimental matrix manageable.
...
| ESM2 + GNN | ESM2 1280-dim → GNN | 25-dim handcrafted | **Main model** |
| ESM2 concat | ESM2 + handcrafted (1313) → GNN | 25-dim handcrafted | Feature complementarity |
| ESM2 + GNN + Uni-Mol | ESM2 → GNN | Uni-Mol 512-dim → GNN | Full pretrained |
...
- [ ] Training runs for all 5 experiments in validation table
...
The key architectural change: instead of predicting `p(bind | protein, ligand)` with a global sigmoid, predict the **relative rank** of each ligand within a protein's ligand pool.
...
- Apply ListMLE: maximise the likelihood of the correct ranking (true binders ranked above decoys)
...
- Normalise scores per-protein: `s_norm(l, p) = (s(l, p) - μ_p) / σ_p` where μ_p, σ_p are the mean and std of protein p's scores across all ligands in the batch
...
- Weight: ramp in during Phase 2/3 of the curriculum, similar to the regression ramp

### 3.2 Protein-Grouped Batching

- Current batching: random sampling of (ligand, protein) pairs → batch size 4
...
- This is necessary for the ranking loss to have enough comparisons
...
- Track attractor Gini coefficient over training epochs — should decrease
- Track per-protein score distributions — attractors should flatten
- Compare with Phase 1 baselines to demonstrate that the ranking loss specifically addresses the attractor problem
...
- Phase 1–3 of curriculum: train ResidueOnlyBind as before (atom module is frozen/inactive)
- Phase 4 (new): unfreeze atom module, add atom-gating loss
...
- Track gate activation rate across substrate classes — expect high activation for organophosphates, low for well-resolved substrates
...
- **BindingDB hydrolases:** Extract EC 3.* entries from BindingDB that do not overlap with BRENDA training set (match by UniProt ID)
- **PDBbind refined set:** Use protein–ligand complexes with experimental binding affinity; filter to hydrolases
- **LP-PDBBind (Leak-Proof):** Use the cleaned, leak-proof PDBBind splits from Li et al. (2026, J Phys Chem B) for rigorous generalisation assessment
...
| *ICML CompBio Workshop* | ~May 2026 | Tight but possible if Phase 1–3 done |
...
Phases 1 and 2 can run in parallel — while baselines train, precompute ESM2 embeddings. Phase 3 depends on Phase 2 (pretrained features). Phase 4 depends on Phase 3 (ranking objective). Phase 5 writing starts early; experiments complete last.
