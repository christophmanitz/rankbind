...
Difference Graph Encoding") to the RankBind setting. Designed + novelty-checked
by a 4-proposal design panel (2026-06-09).
...
reaction is a **reactant → product** transition; Atom-Atom Mapping (AAM) aligns
the *same atom* across the two states; the model encodes the **per-atom feature
difference** `d(v) = c_product(v) − c_reactant(v)`. The difference localises the
**reaction center** (where bonds break/form) and cancels the conserved scaffold
(whose difference ≈ 0). A second GNN then propagates the difference so it
becomes a synergistic, pocket-scale signal.
...
structural win (GIN 0.912 → 0.921); **naive fusion of two unaligned modalities
...
|---------------------------------------|--------------------------------------------------------|
| reactant → product                    | **unbound (free) → bound (coupled)** state             |
| AAM (atom identity across states)     | identity of the same residue / ligand atom — **free**  |
...
exist. Binding's two states are apo (unbound) → holo (bound), but **no holo
structure exists at inference** in RankBind. Rather than *predict/hallucinate*
...
a single weight-shared network, toggling only the cross-modal attention mask:
...
Tensor shapes use the repo's dims: ESM2 per-residue `1280`, ChemBERTa per-token
`384`, projection `d = 256`.
...
3. **Weight-shared bipartite encoder** (the engine): `T=2` transformer blocks
   ...
   attention mask (`A_free` block-diagonal vs `A_coup` cross-open) → `H_free`,
   `H_coup ∈ [B, L+A, 256]`. Cache the coupled cross-attention weights
   `W_cross ∈ [B, heads, L, A]` for the contact map.
...
5. **Difference-graph propagation** (GMSF "second GNN" graft, ablatable
   ...
   weight-shared layer under the coupled mask → propagated `D̃`. **Only `D̃`
   routes into the score; `H_free` is a context channel only.**
...
8. **Integration:** `head_type='deltafield'` in `RankBind.__init__`, exposing the
   same `score_pairs` / `score_triplet` signatures (train/eval/loss/metrics
   untouched). `refresh_scores` is refactored to pre-project `u(P)`, `v(L)`, and
   **cache the ligand-independent free pass `H_free(P)` per protein** (it never
   changes with the ligand) — only the coupled pass + field run per (lig,prot)
   chunk, amortising the 200×200 matrix and the hard-neg refresh. Ship as
   `configs/abl_deltafield.json` extending `default.json`.
...
2. **No additive protein term reaches the scalar.** Remove the ligand → `D̃→0`
   → `score→b0` (one global constant). A per-protein prior is *literally
   unrepresentable* in the score — the formal analog of GMSF's
...
⇒ `Δ == 0` exactly ⇒ `score == b0` for every protein. A unit test asserts this;
the audit metric is Top-10 Jaccard-vs-`null_prot_prior` staying ≈ 0. The
guarantee is *verifiable*, not asserted.
...
device in any DTI/PLI paper); the constituent pieces are borrowed and cited as
such. The paper must contrast head-on with:
...
- **FlowDock** (Morehead & Cheng) — same apo→holo + affinity framing, but a
  separate head on a **generated 3D complex**. DeltaField's coupled pass is a
...
- **MONN / LaMPSite / ICAN / ArkDTA** — residue×atom cross-attention interaction
  maps. DeltaField borrows the map but its **score is read only from the
  difference field**, with the zero-field invariant — those models score from
  the coupled representation directly.

The single defensible novelty: *a single weight-shared bipartite
residue×atom transformer run twice (free vs coupled mask), with the per-node
coupled−free difference as the **sole** scoring substrate **and** the
interaction map, giving a by-construction, CI-testable anti-shortcut guarantee
for ligand-conditional DTI ranking.*
...
- **residue map** `e_res` (+ signed `c_res` = % of predicted affinity,
...
- **Pre-registered go/no-go:** flip the project's own v5b finding that attention
  *avoids* active sites (AUC 0.08–0.21) to **> 0.5** (ideally > 0.7) — i.e. show
  the difference field concentrates *on* the pocket where free attention avoided
  it. Reported honestly as a **secondary** metric; the ranking claim does not
  depend on it.
...
- **Primary loss unchanged:** within-ligand `margin_loss` (k=4, m=1.0) on
  ...
  refreshed every 2–3 epochs (field forward is heavier than a bilinear dot).
  Metric of record stays matrix-MRR / H@5 / H@10; 3 seeds {42,7,1337};
  AdamW/cosine/bf16; frozen PLMs; early-stop on `val_global_auc`.
- **Added field regularisers** (small, ablatable):
  - `L_sparse = λ_s·(mean e_res + mean e_atom)` — localise the interface
    (needs λ annealing + a **floor on total field energy** to avoid collapse).
  - `L_tv = λ_tv·TV(e_res)` along the backbone — contiguous pocket, not speckle.
...
one GPU via `scripts/run_v5_rankbind.sh`. Steps: (1) per-token ChemBERTa cache +
token→atom map; (2) implement `head_type='deltafield'`; (3) refactor
`refresh_scores` (cache free pass per protein); (4) train margin + `L_sparse` +
`L_neg` (λ_tv=0, contact OFF); (5) eval on the 200×200 matrix with the unchanged
`matrix_ranking_metrics` + `null_baselines`.
...
- **(B) Anti-shortcut** — Top-10 Jaccard vs `null_prot_prior` ≤ 0.11 (ideally
  ≈ 0) **and** the zero-field unit test passes exactly.
...
- **Sparsity collapse** (most likely failure): too-strong `L_sparse`/`L_neg` →
  `Δ→0` everywhere → MRR→chance. Mitigate with λ annealing + a field-energy
  floor; monitor field energy in `train_log.jsonl`.
...
- **Inherited pocket anti-correlation** — ESM2 is sequence-trained; the field
  may track biophysics (hydrophobicity) rather than the interface even after
  subtraction (cf. the v5b finding). Pocket-overlap is a *gating* experiment.
- **Compute** — coupled pass + L×A cross-attention over B·k negatives is far
...
- **ChemBERTa BPE ≠ heavy atoms** — atom-level map is approximate; frame as
  coarse, validate against RDKit substructure.
- **Novelty defense** — without the head-on TAPB / MONN / LaMPSite contrast a
  reviewer will call it derivative. Lead with the concrete null-baseline failure
  mode and the CI-tested zero-field theorem, not the abstract "shortcut" label.

---

*Provenance:* design panel `wf_b9969c42-7e2` (4 proposals: CounterBind,
CoupliNet, DeltaDock, PField → novelty-checked → synthesised). Backbone =
