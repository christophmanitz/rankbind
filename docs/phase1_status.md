...
## Results: protein attractor bias is universal

| Model | Architecture | Best AUC | Gini(attractor) | Status |
|-------|-------------|----------|-----------------|--------|
| ResidueOnlyBind_v4 | GNN (dual-encoder) | n/a | 0.9615 | Complete |
...
All seven architectures show strong protein-attractor bias (Gini >= 0.83). Six of seven are above 0.97, so the bias holds across CNN, GCN, Transformer, BAN fusion, and BiLSTM+GCN. GIGN (3D structure-based) is lower but still substantial at 0.83, which suggests spatial interaction features offer partial, but not sufficient, resistance to attractor collapse.

---

## Smoke test (re-implementations): all complete

Simplified re-implementations to validate the data pipeline and demonstrate attractor bias. Jobs: 21250020-21250027 (SLURM, partition=paula, A30 GPU).

- All 7 models trained successfully on 9178-9330 samples (882 proteins, binary classification)
- Score matrices: 100x100 cross-evaluation for all models
...
- Score matrix OOM: reduced N_MATRIX from 300 to 100, added chunked evaluation
...
- [ ] Install DGL in a separate venv for DrugBAN (`scripts/setup_drugban_venv.sh`)
- [ ] Run DiffDock for GIGN 3D structures (903 proteins x ligands)
